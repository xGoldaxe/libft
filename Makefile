SRCS	=  ft_isalpha.c ft_memchr.c  ft_memmove.c ft_strlcpy.c \
ft_bzero.c   ft_isascii.c ft_isprint.c ft_memcmp.c  ft_memset.c  ft_strlen.c \
ft_isalnum.c ft_isdigit.c ft_memcpy.c  ft_strlcat.c ft_strnlen.c \
ft_tolower.c ft_toupper.c ft_strchr.c ft_strrchr.c ft_strncmp.c ft_strnstr.c \
ft_isspace.c ft_atoi.c ft_calloc.c ft_strdup.c ft_substr.c \
ft_strjoin.c ft_strtrim.c ft_include.c ft_strreverse.c ft_split.c ft_itoa.c \
ft_strmapi.c ft_putchar_fd.c ft_putstr_fd.c ft_putendl_fd.c ft_putnbr_fd.c ft_striteri.c

HEADERS	= libft.h 

OBJS	= ${SRCS:.c=.o}

NAME	= libft.a

CC		= gcc

LIBC	= ar -rc

RM		= rm -f

CFLAGS	= -Wall -Wextra -Werror


%.o:		%.c	
				${CC} -I. -o $@ -c $? ${CFLAGS}

${NAME}:	${OBJS}
				${LIBC} ${NAME} ${OBJS}

all:		${NAME}

clean:
				${RM} ${OBJS}

fclean:		clean
				${RM} ${NAME}

re:				fclean all

PHONY:		all clean fclean re
